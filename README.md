...
Класс, который использует композицию, чтобы выполнить запросы к серверу с помощью методов класса Api. Он отвечает за получение списка товаров с сервера и отправку данных заказа.
